    ...
    IZonneplanTariffReader zonneplanTariffReader) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                logger.LogInformation("ZonneplanScheduleService running at {time}", DateTimeOffset.Now);
                
                var currentDateTime = DateTimeOffset.UtcNow;
                var lastUpdated = config.CurrentValue.Zonneplan.Authentication.LastUpdated! ?? DateTimeOffset.UtcNow.AddDays(-1);
                var expiresIn = config.CurrentValue.Zonneplan.Authentication.ExpiresIn;
                var timeUntilExpiry = lastUpdated.AddSeconds(expiresIn) - currentDateTime;

                // if token expires in less than 4 hours, refresh it
                if (timeUntilExpiry.TotalHours <= 4)
                {
                    logger.LogInformation("Zonneplan token expiring, refreshing...");

                    var response = await zonneplanAuthentication.RefreshTokenAsync(config.CurrentValue.Zonneplan.Authentication.RefreshToken!);

                    if (response != null)
                    {
                        config.CurrentValue.Zonneplan.Authentication.AccessToken = response.Value<string>("access_token")!;
                        config.CurrentValue.Zonneplan.Authentication.RefreshToken = response.Value<string>("refresh_token")!;
                        ...
                        config.CurrentValue.Zonneplan.Authentication.LastUpdated = DateTimeOffset.Now;
                        await configWriter.WriteAsync(config.CurrentValue);
                ...
                {
                    logger.LogInformation("Token valid, no refresh. Expires in {min} min.", timeUntilExpiry.TotalMinutes);
                }
            }
            catch (Exception ex)
            {
            ...
            finally
            {
                ...
                else
                {
                    ...
                    await Task.Delay(config.CurrentValue.Zonneplan.RefreshIntervalMinutes * 60 * 1000, stoppingToken); // wait one hour
                }
            }
        }
    }
